...
from PredNetConvGRU.Encoder import Encoder
from PredNetConvGRU.Decoder import Decoder
import numpy as np 
import os
import cv2 
...
class PrednetModel(nn.Module):
    ...
    def get_init_Elt_tensor(self,state):
        return Variable(torch.randn(state[0],state[1],int(state[2]),int(state[3]))).cuda()
